# ReaUI Builder — User Guide, Version 1.0.40

[HTML version — open locally](ReaUI_Builder_Manual_en_v1_0_39.html)
...
This guide describes **1.0.40**, incorporating the changes made since **1.0.0**. It explains the current workflow; older workarounds are included only where they still apply.
...
See [§4.6](#manual-4-6-containers-and-nesting) for shared container behavior: a 24 px editor strip, position-based nesting, adjustment of child positions during resizing, and the choice of retaining or deleting content when a filled container is removed. The sections below describe the differences between container types.
...
StyleRegion applies style overrides to its child widgets without affecting the rest of the layout. Use it to style a region with shared overrides. For a quick color change to selected objects, the object context menu also provides Color commands.
...
Each available fill/stroke row has its own **disable** checkbox. Check it to turn that component off without losing its color. The controls are independent: a shape may have a fill, an outline, both, or neither.

Stroke can also be disabled for Line and ring-mode Arc. A fully disabled or transparent drawing is still in the project; select it through Elements. Fill and stroke colors are edited independently; there is no color-link control in 1.0.40.
...
> **Preview limitations:**
>
> - ColorEdit and other native controls may not reflect every flag or interaction. Preview is a layout approximation, not a running ReaImGui window.
>
...
| **New Project** | Starts an empty 600 × 400 layout, retaining current title, background, theme, grid, and snapping settings. |
...
Builder keeps recovery drafts in this browser’s storage. Use **File → Restore projects…** to inspect and restore drafts from closed sessions; there is no automatic restore question on startup. Projects still open in another active tab are excluded from the list.

A successful direct save clears the completed draft. If new edits arrive during writing, their draft remains. The download fallback retains recovery data because the browser does not tell Builder whether the download was completed or canceled. A restored draft remains something you should save to a project file.
...
Recovery and custom theme storage are tied to a browser and can be lost when site data is cleared. Local-file storage behavior also varies between browsers. Keep a JSON project as your durable, portable copy.
...
The following boundaries still apply to **1.0.40**.

<a name="manual-not-implemented"></a>

### Not implemented

- Editing Combo/ListBox item lists in Builder: the export contains five placeholder items. Replace their strings in Lua; selection state and widget calls are already generated.
- Importing generated Lua back into Builder. Save JSON to preserve an editable project.
- DSP, application logic, and finished event handlers. Fill in the generated TODOs in your script.
- General rotation handles or a rotation property for drawings. Arc angles and Triangle orientation are separate, supported properties.
- A separate theme-file import/export workflow. The active theme is included in project JSON.
...
- Container inspectors still show **hint**, but their exporter does not emit a container tooltip. For hover help, use a supported leaf widget or HelpMarker.
...
This table follows the fields available for each of the **50 types** in 1.0.40. Names use the inspector’s labels: **hint** is a hover tooltip, while **placeholder** belongs to InputTextWithHint. Main **name** and conditional **parent** information are not repeated. Variant is chosen before placement; fixed size axes remain read-only.

TabItem and TableCell are structural types managed by their parent. Their fields describe the internal type; edit tab and table structure through TabBar/Table. Shared rows for a multiple selection can be narrower than this single-type list. The hint row on containers is shown by the inspector but not emitted in Lua.
...
| `Panel` | label / text, hint (not exported), border, h-scroll, size, position |
| `Group` | label / text, hint (not exported), size, position |
| `StyleRegion` | label / text, hint (not exported), text color, frame bg, button color, rounding, selectable align, font, size, position |
| `CollapsingHeader` | label / text, hint (not exported), bullet instead of arrow, size, position |
| `TreeNode` | label / text, hint (not exported), bullet instead of arrow, hdr color, size, position |
| `TabBar` | tabs, label / text, hint (not exported), size, position |
| `TabItem` | label / text, hint (not exported), size, position |
| `Table` | table grid / columns / row heights / table flags, label / text, hint (not exported), size, position |
| `TableCell` | label / text, hint (not exported), size, position |
...
These excerpts come from a 1.0.40 export of a **600 × 400** layout with Button_1 (“Play”) at (40, 60) and Slider_2 (“Gain”) at (40, 130). They show the current position/state structure. They are excerpts, not a complete runnable script; use Export to obtain the complete file.
...
*ReaUI Builder — User Guide for Version 1.0.40. Updated from the English 1.0.3 guide against the current release source and project change records covering development since 1.0.0. Updated 14 September 2026.*
